...
use crate::core::models::plant::{
    CreatePlantRequest, PlantResponse, PlantStats, PlantVariable, UpdatePlantRequest, VariableType,
};
...
fn invalid_argument(message: impl Into<String>) -> ErrorDto {
    ErrorDto::from(AppError::InvalidArgument(message.into()))
}

fn expect_object<'a>(value: &'a Value, context: &str) -> Result<&'a serde_json::Map<String, Value>, ErrorDto> {
    value.as_object()
        .ok_or_else(|| invalid_argument(format!("{context} deve ser um objeto")))
}

fn expect_array<'a>(value: &'a Value, context: &str) -> Result<&'a Vec<Value>, ErrorDto> {
    value.as_array()
...
) -> Result<&'a serde_json::Map<String, Value>, ErrorDto> {
    match root.get("meta") {
...
fn expect_string(value: Option<&Value>, context: &str) -> Result<String, ErrorDto> {
    value.and_then(Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| invalid_argument(format!("{context} deve ser uma string")))
}

fn expect_number(value: Option<&Value>, context: &str) -> Result<f64, ErrorDto> {
...
fn compute_imported_variable_stats(
    ...
    let min = values.iter().fold(f64::INFINITY, |acc, value| acc.min(*value));
    let max = values
        .iter()
...
fn parse_variable_type(value: &str) -> Result<VariableType, ErrorDto> {
...
fn parse_registry_variable(value: &Value, index: usize) -> Result<PlantVariable, ErrorDto> {
    ...
    let name = expect_string(variable_obj.get("name"), &format!("variables[{index}].name"))?;
    ...
    let pv_max = get_value_by_keys(variable_obj, &["pv_max", "pvMax"])
        .and_then(Value::as_f64)
        .unwrap_or(100.0);
    let linked_sensor_ids = get_value_by_keys(variable_obj, &["linked_sensor_ids", "linkedSensorIds"])
...
fn parse_registry_driver(
...
) -> Result<Option<ImportedWorkspaceDriverResponse>, ErrorDto> {
    ...
    let config = match driver_obj.get("config") {
        ...
        Some(value) => {
            serde_json::from_value::<HashMap<String, SchemaFieldValue>>(value.clone()).map_err(
                |error| invalid_argument(format!("driver.config inválido: {error}")),
            )?
        }
    };
...
pub fn open_plant_file(request: ImportFileRequest) -> Result<OpenPlantFileResponse, ErrorDto> {
    ...
    for (index, actuator) in actuators.iter().enumerate() {
        ...
        let name = expect_string(actuator_obj.get("name"), &format!("actuators[{index}].name"))?;
    ...
    for (index, setpoint) in setpoints.iter().enumerate() {
        ...
        let setpoint_id = expect_string(setpoint_obj.get("id"), &format!("setpoints[{index}].id"))?;
        let sensor_id =
            expect_string(setpoint_obj.get("sensorId"), &format!("setpoints[{index}].sensorId"))?;
    ...
    for (sample_index, sample) in data.iter().enumerate() {
        ...
        point.insert(
            "time".into(),
            expect_number(sample_obj.get("time"), &format!("data[{sample_index}].time"))?,
        );

        let sensors_record = expect_object(
            sample_obj
                .get("sensors")
                .ok_or_else(|| invalid_argument(format!("data[{sample_index}].sensors ausente")))?,
